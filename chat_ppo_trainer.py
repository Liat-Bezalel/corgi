...
class PPOTrainer(BaseTrainer):
    ...
    def _generate_batched(
        ...
        **generation_kwargs,
        ...
        for i in range(0, len(query_tensors), batch_size):
            ...
            padded_inputs['attention_mask'] = (padded_inputs['input_ids'] != self.tokenizer.pad_token_id).long()
    ...
    def step(
        ...
        queries: List[torch.LongTensor],
        responses: List[torch.LongTensor],
        chats_input_ids: torch.LongTensor, 
        ...
        scores = self._step_safety_checker(scores)
        ...
        model_inputs = self.prepare_model_inputs(chats_input_ids)
        ...
        with torch.no_grad():
            all_logprobs, logits_or_none, values = self.batched_forward_pass(
                len(queries), self.model, model_inputs, return_logits=full_kl_penalty
            )
            ...
            if self.is_peft_model and hasattr(
                ...
                ref_logprobs, ref_logits_or_none, _ = self.batched_forward_pass(
                    len(queries), self.ref_model, model_inputs, return_logits=full_kl_penalty
                )
            ...
            for backward_batch_start in range(0, bs, self.config.backward_batch_size):
                ...
                for mini_batch_start in range(0, self.config.backward_batch_size, self.config.mini_batch_size):
                    ...
                    with self.accelerator.accumulate(self.model):
                        ...
                        logprobs, logits, vpreds = self.batched_forward_pass(
                            len(mini_batch_inds),
                            self.model,
                            model_inputs,
                            return_logits=True,
    ...
    def prepare_model_inputs(self, chats_input_ids: torch.Tensor):
        # if self.is_encoder_decoder:
        ...
        # else:
        input_ids = chats_input_ids
        input_data = self.data_collator(
            [{"input_ids": ids, "attention_mask": ids != self.tokenizer.pad_token_id} for ids in input_ids]
        ).to(self.current_device)
